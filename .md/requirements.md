...
- Popup thông tin tài khoản của người dùng đã đăng nhập có mục quản lý đặt cọc để xem lại các đơn đặt cọc của chính mình, tìm theo mã đơn, tên/hãng xe hoặc số điện thoại, lọc theo trạng thái `pending`, `confirmed`, `completed`, `cancelled_after_deposit`, `cancelled`, `expired`, và nhấn vào từng đơn để xem chi tiết thông tin xe, khách hàng, thanh toán, chứng từ chuyển khoản, ngân hàng, hạn giữ chỗ, ghi chú và lịch sử xử lý; với đơn `pending` dùng VNPay sandbox chưa có kết quả thất bại/hủy, khách có thể bấm "Tiếp tục thanh toán VNPay" để hệ thống tạo lại URL thanh toán và chuyển sang cổng VNPay; nếu VNPay đã trả kết quả thất bại hoặc khách hủy/quay về từ cổng thanh toán thì đơn chuyển sang `cancelled` và khách phải tạo đơn mới để thanh toán lại; menu tài khoản thay nút giỏ hàng bằng nút "Quản lý đặt cọc".
- Cho phép người dùng liên hệ mua xe hoặc đặt lịch xem xe
- Khách hàng đã đăng nhập có widget nhắn tin realtime với cửa hàng trên các trang công khai. Khách có thể tạo cuộc hội thoại theo chủ đề, xem lại lịch sử, gửi tin mới và theo dõi badge chưa đọc; khách chỉ được truy cập hội thoại của chính mình.
- Trang quản trị có mục "Tin nhắn khách hàng" cho `staff` và `admin` xem hộp thư chung, lọc theo trạng thái `open`, `in_progress`, `closed`, nhận xử lý, trả lời và đóng/mở lại hội thoại. Khi khách nhắn tiếp vào hội thoại đã đóng, hệ thống tự chuyển lại `open`.
- Lịch sử hội thoại và tin nhắn phải lưu trong SQLite; REST API dùng để tạo/tải lịch sử và làm fallback, Socket.IO chỉ dùng để đẩy tin mới/badge tức thời. Socket phải xác thực bằng session cookie hiện tại, kiểm tra quyền theo từng hội thoại và không tin `user_id` do frontend gửi lên.
...
- Trang `/thanh-toan-dat-coc` chỉ tập trung tạo và thanh toán đơn đặt cọc, không hiển thị form tra cứu riêng. Chức năng tìm kiếm được đặt trong mục "Quản lý đặt cọc" của tài khoản, cho phép lọc các đơn của chính người dùng theo mã đơn, tên/hãng xe hoặc số điện thoại và kết hợp với bộ lọc trạng thái `pending`, `confirmed`, `completed`, `cancelled_after_deposit`, `cancelled`, `expired`. Trang thanh toán vẫn có khối giải thích trạng thái đơn để khách biết cần chuyển khoản/thanh toán VNPay, chờ xác nhận, kiểm tra hoàn cọc hoặc tạo đơn mới khi quá hạn hay khi VNPay thất bại/hủy. Sau khi tạo đơn đang `pending`, frontend lưu tạm mã đơn và số điện thoại trong `localStorage`; nếu khách reload trang hoặc quay về từ VNPay, màn hình chờ/kết quả được khôi phục và tự kiểm tra lại trạng thái, còn khi đơn `confirmed`, `cancelled` hoặc `expired` thì dữ liệu tạm được xóa.
...
- Trang quản trị có mục "Quản lý đặt cọc" cho `staff` và `admin` xem danh sách đơn đặt cọc, tìm kiếm/lọc theo trạng thái và xử lý các bước vận hành được giao. Chỉ `admin` được cấu hình tài khoản ngân hàng nhận cọc, mức tiền đặt cọc, giới hạn tối thiểu/tối đa, thời gian giữ xe, yêu cầu chứng từ và nội dung chính sách đặt cọc/hủy cọc/hoàn cọc.
...
- Trang quản trị có mục "Yêu cầu tư vấn" để `staff` và `admin` xem, tìm kiếm và cập nhật trạng thái; chỉ `admin` được xóa yêu cầu tư vấn của khách hàng.
...
- Trang quản trị có mục "Thông tin khuyến mại" nằm dưới mục "Khách hàng" để `staff` xem nội dung hiện có; chỉ `admin` được viết bài, tải/cắt ảnh, sửa, xóa, bật/tắt hiển thị và sắp xếp thứ tự trên trang chủ.
...
- Trang quản trị có mục "Quản lý blog" để `staff` xem danh sách và trạng thái bài viết; chỉ `admin` được tạo, sửa, xóa, tải/cắt ảnh, xuất bản/ẩn bài, bật hiển thị Home và đánh dấu bài nổi bật.
...
- Trang quản trị có mục "Quản lý đăng bán xe" cho `staff` và `admin` xem thông tin xe khách gửi, ảnh xe, số điện thoại/email để sale gọi lại và tìm kiếm/lọc theo trạng thái; chỉ `admin` được chốt giá, duyệt nhập kho hoặc từ chối/xóa tin.
- Khi `admin` duyệt thông tin đăng bán xe, bắt buộc nhập giá chốt với khách và giá bán trên hệ thống, mỗi giá gồm bản hiển thị và dạng số. Backend dùng giá bán trên hệ thống để tạo xe mới trong bảng kho xe `cars` với trạng thái còn xe, cập nhật bài đăng bán sang `approved` và gửi thông báo duyệt cho khách hàng. Giá khách mong muốn chỉ giữ lại để tham khảo, giá chốt với khách dùng làm giá vốn/quản trị nội bộ, không dùng làm giá bán công khai.
...
- Khi `admin` từ chối thông tin đăng bán xe, bắt buộc nhập lý do; backend gửi thông báo lý do từ chối cho khách hàng, xóa bài đăng bán xe bị từ chối và khách hàng cần đăng lại bài nếu muốn OkXe kiểm tra lại.
...
- Trang quản trị có mục "Quản lý tin mua xe" cho `staff` và `admin` xem, tìm kiếm, lọc và liên hệ khách; chỉ `admin` được duyệt, từ chối hoặc xóa tin mua xe. Việc cập nhật trạng thái thực hiện qua popup có ghi chú xử lý, khi từ chối phải nhập lý do và khi đổi khỏi trạng thái từ chối không được giữ lại lý do từ chối cũ.
...
- Quyền `staff` được xem, thêm và sửa nội dung/ảnh/thông số xe nhưng không được xóa hoặc thay đổi trạng thái xe. Quyền `admin` có toàn quyền kho xe trong phạm vi ràng buộc đặt cọc, đồng thời quản lý tài khoản nhân viên, bao gồm tạo tài khoản nhân viên/admin, chỉnh sửa họ tên/email/mật khẩu/role, đổi role nhanh và xóa tài khoản.
- `staff` được xem khách hàng và dữ liệu liên hệ phục vụ tư vấn nhưng không được sửa trường nhạy cảm hoặc quản lý tài khoản. Chỉ `admin` được sửa hồ sơ khách hàng và quản lý tài khoản.
- `staff` chỉ được xem bài khuyến mại và bài viết blog; chỉ `admin` được tạo, sửa, xuất bản/ẩn, sắp xếp và xóa nội dung công khai.
- Các thao tác xóa xe, lịch lái thử, yêu cầu tư vấn, tin mua xe, khuyến mại và blog chỉ dành cho `admin`; giao diện phải ẩn thao tác với `staff` và backend vẫn phải trả `403` nếu gọi API trực tiếp.
- `staff` được xác nhận nhận cọc và cập nhật trạng thái hợp lệ theo luồng, nhưng chỉ `admin` được xử lý `cancelled_after_deposit`, ghi nhận hoàn tiền, xem báo cáo tài chính đặt cọc và sửa cấu hình thanh toán/chính sách.
...
### Nhắn tin realtime khách hàng

- Bảng `conversations` lưu khách hàng, chủ đề, ngữ cảnh liên quan, nhân viên phụ trách, trạng thái, mốc đọc gần nhất và thời gian tin cuối.
- Bảng `conversation_messages` lưu người gửi, vai trò người gửi, nội dung tối đa 2.000 ký tự và thời gian gửi. MVP chỉ hỗ trợ văn bản, chưa hỗ trợ file đính kèm hoặc trạng thái đang nhập.
- Một cuộc hội thoại được phát qua phòng Socket.IO `conversation:{id}`; server phải kiểm tra chủ sở hữu hoặc quyền nhân viên trước khi cho tham gia phòng hay gửi tin.
- Nếu realtime mất kết nối, frontend gửi tin qua REST API và tải lại lịch sử sau khi kết nối lại. Database luôn là nguồn dữ liệu chính thức.

### KPI / Hiệu quả kinh doanh sale

- Trang quản trị có mục **KPI / Hiệu quả kinh doanh** cho `staff` xem KPI của chính mình ở chế độ chỉ đọc. Chỉ `admin` được xem toàn bộ đội ngũ, tạo, sửa, hủy KPI, cấu hình chỉ tiêu, khóa/mở kỳ và xử lý chi thưởng.
- KPI `direct_sale` vẫn tự động phát sinh khi admin chọn nhân viên và chuyển xe bán trực tiếp sang `Xe đã bán`. Riêng duyệt tin bán xe của khách vào kho và hoàn tất đơn đặt cọc không yêu cầu chọn nhân viên, không tạo KPI ngay; giao dịch được tự động đưa vào danh sách nguồn chờ tại mục KPI để admin chọn đúng nhân viên rồi mới ghi nhận KPI `acquisition` hoặc `sale`. Chính sách nhân viên, vai trò và trạng thái khóa kỳ chỉ được kiểm tra ở bước admin ghi KPI, không được chặn thao tác nhập kho hoặc hoàn tất đơn đặt cọc. Đơn mới xác nhận cọc (`confirmed`) không được tính KPI/doanh số.
- KPI lưu `business_date` theo ngày giao dịch hoàn tất, `policy_period` theo tháng của ngày nghiệp vụ và `creation_mode` (`automatic`/`manual`). Bộ lọc báo cáo và công thức hoàn thành dùng `business_date`, không dùng ngày admin thao tác. Hệ thống lưu ảnh chụp dữ liệu giao dịch và mức thưởng tại thời điểm phát sinh để báo cáo lịch sử không đổi theo dữ liệu xe/đơn về sau.
...
- Admin có thể điều chỉnh nhân viên và ghi chú của KPI còn hiệu lực khi kỳ đang mở; tiền thưởng luôn lấy từ chính sách của `policy_period`. Khi hủy phải nhập lý do. KPI hủy không bị xóa mà giữ lịch sử và không còn tính vào thống kê.
- Dashboard KPI mặc định báo cáo theo tháng hiện tại và cho phép lọc khoảng ngày, nhân viên, loại KPI, trạng thái thưởng; số liệu hiện tại được so sánh với kỳ liền trước có cùng độ dài.
- Dashboard hiển thị số xe bán, doanh số, chênh lệch mua–bán đã chốt, biên chênh lệch, hoa hồng chờ chi, số xe nhập và xu hướng doanh số; cho phép xuất CSV theo đúng bộ lọc hiện tại. Vì chưa quản lý chi phí sửa chữa/vận chuyển/vận hành, UI không gọi chênh lệch mua–bán là lợi nhuận thực tế.
- Báo cáo và bảng xếp hạng phải tách theo vai trò: bán xe xếp theo mức đạt số xe, nhập xe xếp theo mức đạt chênh lệch mua–bán, bảng kiêm nhiệm chỉ gồm nhân viên `both` và dùng mức hoàn thành tổng hợp.
- Chênh lệch mua–bán chỉ tính cho xe có KPI nhập hợp lệ và `purchase_price_value > 0`; giao dịch thiếu KPI/giá nhập phải được cảnh báo và không được giả định giá vốn bằng 0.
- Admin cấu hình vai trò KPI cho từng `nhân viên + tháng`: `sales_only` (chỉ bán xe), `acquisition_only` (chỉ nhập xe) hoặc `both` (vừa nhập vừa bán). Backend phải chặn ghi KPI nhập/bán không đúng vai trò đang áp dụng trong kỳ.
- KPI bán xe dùng mục tiêu số xe và mức hoa hồng cố định trên mỗi xe. Khi ghi nhận `sale` hoặc `direct_sale`, backend tự lấy `commission_per_sale` của nhân viên trong tháng để lưu vào `reward_amount`; admin không nhập hoa hồng thủ công. Vai trò có bán xe bắt buộc cấu hình hoa hồng lớn hơn 0; nhân viên chưa có chính sách tháng không được chọn để ghi KPI. Doanh số chỉ dùng báo cáo, không tham gia công thức mức hoàn thành.
- KPI nhập xe có mức thưởng cố định riêng theo `nhân viên + tháng`, lưu tại `acquisition_reward_per_vehicle`; vai trò có nhập xe bắt buộc cấu hình mức thưởng lớn hơn 0. Khi tin bán xe của khách được duyệt, có giá chốt và xe đã vào kho, giao dịch xuất hiện trong nguồn chờ KPI; admin chọn nhân viên nhập xe rồi ghi nhận KPI `acquisition`. Backend tự lấy mức thưởng nhập của nhân viên để lưu vào `reward_amount` với trạng thái mặc định `pending`, không cần chờ xe được bán.
- Chênh lệch mua–bán vẫn được chốt sau khi cùng `car_id` có KPI bán hợp lệ để đánh giá hiệu quả nhập xe, nhưng không làm thay đổi thưởng nhập đã ghi nhận. Nhân viên `both` vừa nhập vừa bán cùng một xe được nhận đồng thời thưởng nhập cố định và hoa hồng bán cố định. Không cho hủy KPI nhập khi thưởng nhập đã ở trạng thái `paid` cho tới khi admin chuyển lại `pending` để đối soát; hủy KPI bán không ảnh hưởng thưởng nhập đã phát sinh hợp lệ.
- Mức hoàn thành bán xe bằng `số xe bán / mục tiêu xe bán`; mức hoàn thành nhập xe bằng `chênh lệch mua–bán đã chốt / mục tiêu chênh lệch`. Vai trò không áp dụng hoặc mục tiêu bằng 0 hiển thị `N/A` và không tham gia mức hoàn thành tổng hợp; nhân viên `both` lấy trung bình hai chỉ tiêu có thiết lập.
- Bộ lọc trạng thái chi thưởng chỉ lọc sổ chi trả, không được làm thay đổi số xe, doanh số, chênh lệch hoặc tỷ lệ hoàn thành KPI. Chỉ số xe nhập chờ bán trong kỳ chỉ tính nguồn nhập có `business_date` nằm trong kỳ; tồn xe nhập chưa bán toàn thời gian được lưu riêng dưới dạng backlog.
- Mỗi tháng có trạng thái `open`/`locked`. Kỳ đã khóa không cho sửa chính sách hoặc tạo/sửa/hủy thành tích KPI; quy trình duyệt và chi thưởng vẫn tiếp tục được sau khi chốt kỳ. Admin có thể mở khóa lại nhưng phải nhập lý do. Mỗi lần tạo/cập nhật chính sách phải ghi một snapshot vào lịch sử chính sách, gồm người thay đổi, thời gian và toàn bộ giá trị sau thay đổi.
- Quy trình chi thưởng gồm `pending` (chờ duyệt) → `approved` (đã duyệt) → `paid` (đã chi); từ `approved` có thể trả về `pending`. Khi xác nhận `paid` bắt buộc nhập mã phiếu chi/mã giao dịch. Hệ thống lưu người duyệt, người chi, thời gian, mã tham chiếu và lịch sử từng lần chuyển trạng thái.
- Báo cáo tổng kết KPI lấy trọn tháng theo `targetPeriod`, không bị thay đổi bởi bộ lọc loại KPI hoặc trạng thái chi thưởng. Mỗi dòng hiển thị vai trò, mục tiêu/kết quả bán xe, mục tiêu/kết quả chênh giá nhập xe, tỷ lệ hoàn thành tổng hợp, tổng thưởng, đã chi và còn phải chi; cho phép xuất CSV và dùng hộp thoại in để in/lưu PDF.
- Xếp loại tự động theo tỷ lệ hoàn thành tổng hợp: `>= 120%` Xuất sắc, `>= 100%` Tốt, `>= 80%` Đạt, `>= 60%` Cần cải thiện, `< 60%` Không đạt; trường hợp chưa có chỉ tiêu hợp lệ hiển thị Chưa xếp loại. Xếp loại chỉ phục vụ đánh giá, không tự thay đổi mức thưởng/hoa hồng.
- Trung tâm cảnh báo KPI phải liệt kê xe bán thiếu KPI nhập, xe nhập đang chờ bán, nhân viên chưa có chính sách kỳ và chính sách bán thiếu hoa hồng; mỗi cảnh báo có hành động mở đúng giao dịch, form ghi nhận hoặc dòng cấu hình liên quan.

### Thay đổi database mới nhất

- Bảng thêm: `sales_kpi_records`, `sales_kpi_targets`, `sales_kpi_periods`, `sales_kpi_policy_history`, `sales_kpi_reward_history`.
...
- `sales_kpi_targets` lưu kỳ tháng `YYYY-MM`, nhân viên, `kpi_role`, mục tiêu số xe bán, `commission_per_sale`, `acquisition_reward_per_vehicle`, mục tiêu chênh lệch mua–bán và admin cập nhật; tên cột `gross_profit_target` và `revenue_target` được giữ tương thích dữ liệu cũ nhưng doanh số không còn tham gia chấm KPI. Ràng buộc `UNIQUE (period, sale_user_id)` để mỗi nhân viên chỉ có một bộ chính sách trong một tháng.
