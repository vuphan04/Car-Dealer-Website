(function () {
    ...
    const renderNotFound = () => {
        ...
        articlePage.innerHTML = `
            <section class="article-not-found">
                ...
                <a href="/Car-Dealer-Website/blog"><i class="bx bx-left-arrow-alt" aria-hidden="true"></i>Quay lại Blog</a>
            </section>
        `;
    };
    ...
    const relatedCards = relatedPosts.map((item) => `
        <article class="article-related-card">
            <a href="/Car-Dealer-Website/blog/${encodeURIComponent(item.slug)}" class="article-related-card__media">
                <img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.imageAlt || item.title)}" loading="lazy">
            </a>
            <div>
                <span>${escapeHtml(item.category)}</span>
                <h3><a href="/Car-Dealer-Website/blog/${encodeURIComponent(item.slug)}">${escapeHtml(item.title)}</a></h3>
        ...
        <article class="article-layout">
            <header class="article-header">
                <nav class="article-breadcrumb" aria-label="Đường dẫn bài viết">
                    <a href="/Car-Dealer-Website/">Trang chủ</a>
                    <i class="bx bx-chevron-right" aria-hidden="true"></i>
                    <a href="/Car-Dealer-Website/blog">Blog</a>
                    <i class="bx bx-chevron-right" aria-hidden="true"></i>
            ...
            <div class="article-content">
                ...
                <a href="/Car-Dealer-Website/blog" class="article-back"><i class="bx bx-left-arrow-alt" aria-hidden="true"></i>Quay lại danh sách bài viết</a>
            </div>
        </article>
